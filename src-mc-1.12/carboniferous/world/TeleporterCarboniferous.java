...
public class TeleporterCarboniferous extends Teleporter {
    ...
    protected final Long2ObjectMap<TeleporterCarboniferous.PortalPosition> destinationCoordinateCache = new Long2ObjectOpenHashMap<TeleporterCarboniferous.PortalPosition>(4096);

    public TeleporterCarboniferous(WorldServer worldIn) {
    	super(worldIn);
        this.world = worldIn;
    ...
    public void placeInPortal(Entity entityIn, float rotationYaw) {
        if(this.world.provider.getDimensionType().getId() != CarboniferousModAPI.DIMENSION_TYPE.getId())
        {
    ...
    public boolean placeInExistingPortal(Entity entityIn, float rotationYaw) {
        ...
        {
            TeleporterCarboniferous.PortalPosition teleporter$portalposition = (TeleporterCarboniferous.PortalPosition)this.destinationCoordinateCache.get(l);
            d0 = 0.0D;
            ...
            for (int i1 = -128; i1 <= 128; ++i1)
            {
                ...
                for (int j1 = -128; j1 <= 128; ++j1)
                {
                    for (BlockPos blockpos1 = blockpos3.add(i1, this.world.getActualHeight() - 1 - blockpos3.getY(), j1); blockpos1.getY() >= 0; blockpos1 = blockpos2)
                    {
                        ...
                        {
                            for (blockpos2 = blockpos1.down(); this.world.getBlockState(blockpos2).getBlock() == ModBlocks.PORTAL; blockpos2 = blockpos2.down())
                            {
                                blockpos1 = blockpos2;
                            }
        ...
        if (d0 >= 0.0D)
        {
            if (flag)
            {
                this.destinationCoordinateCache.put(l, new TeleporterCarboniferous.PortalPosition(blockpos, this.world.getTotalWorldTime()));
            }
            ...
            {
                ((EntityPlayerMP)entityIn).connection.setPlayerLocation(d5, 200, d7, entityIn.rotationYaw, entityIn.rotationPitch);
            }
            else
            {
                entityIn.setLocationAndAngles(d5, 200, d7, entityIn.rotationYaw, entityIn.rotationPitch);
            }
    ...
    public boolean makePortal(Entity entityIn)
    {
        ...
        IBlockState iblockstate = ModBlocks.PORTAL.getDefaultState();
    ...
    public void removeStalePortalLocations(long worldTime)
    {
        if (worldTime % 100L == 0L)
        {
            ...
            ObjectIterator<TeleporterCarboniferous.PortalPosition> objectiterator = this.destinationCoordinateCache.values().iterator();

            while (objectiterator.hasNext())
            {
                TeleporterCarboniferous.PortalPosition teleporter$portalposition = (TeleporterCarboniferous.PortalPosition)objectiterator.next();
