...
public class BuildcraftApi {
     ...
     *            the block meta data
     **/
    public void addBuildcraftFacade(Block block, int metadata) {
        try {
            addFacade.get().invoke(null, new ItemStack(block, 1, metadata));
            LogHelper.log(Level.INFO,
                    "(Buildcraft Plugin) Registred a facade (" + block + ", "
                            + metadata + ")");
        } catch (Exception e) {
