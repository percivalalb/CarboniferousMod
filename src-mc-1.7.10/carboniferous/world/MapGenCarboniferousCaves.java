...
public class MapGenCarboniferousCaves extends MapGenBase {
  

	
    protected void func_151542_a(long seed, int x, int z, Block[] p_151542_5_, double p_151542_6_, double p_151542_8_, double p_151542_10_)
    {
        this.func_151541_a(seed, x, z, p_151542_5_, p_151542_6_, p_151542_8_, p_151542_10_, 1.0F + this.rand.nextFloat() * 6.0F, 0.0F, 0.0F, -1, -1, 0.5D);
    }

    protected void func_151541_a(long seed, int chunkX, int chunkZ, Block[] data, double p_151541_6_, double p_151541_8_, double p_151541_10_, float p_151541_12_, float p_151541_13_, float p_151541_14_, int p_151541_15_, int p_151541_16_, double p_151541_17_)
    {
        ...
        for (boolean flag = random.nextInt(6) == 0; p_151541_15_ < p_151541_16_; ++p_151541_15_)
        {
            ...
            {
                this.func_151541_a(random.nextLong(), chunkX, chunkZ, data, p_151541_6_, p_151541_8_, p_151541_10_, random.nextFloat() * 0.5F + 0.5F, p_151541_13_ - ((float)Math.PI / 2F), p_151541_14_ / 3.0F, p_151541_15_, p_151541_16_, 1.0D);
                this.func_151541_a(random.nextLong(), chunkX, chunkZ, data, p_151541_6_, p_151541_8_, p_151541_10_, random.nextFloat() * 0.5F + 0.5F, p_151541_13_ + ((float)Math.PI / 2F), p_151541_14_ / 3.0F, p_151541_15_, p_151541_16_, 1.0D);
                return;
            }
            ...
            if (flag2 || random.nextInt(4) != 0)
            {
                ...
                if (p_151541_6_ >= d4 - 16.0D - d6 * 2.0D && p_151541_10_ >= d5 - 16.0D - d6 * 2.0D && p_151541_6_ <= d4 + 16.0D + d6 * 2.0D && p_151541_10_ <= d5 + 16.0D + d6 * 2.0D)
                {
                        ...
                        for (int l2 = k4; !flag3 && l2 < j2; ++l2)
                        {
                            for (int i3 = i2 + 1; !flag3 && i3 >= j4 - 1; --i3)
                            {
                                ...
                                if (i3 >= 0 && i3 < 256)
                                {
                                    ...
                                    if (isOceanBlock(data, z, x, i3, l2, chunkX, chunkZ))
                                    {
                                        flag3 = true;
                                    }

                                    if (i3 != j4 - 1 && x != i4 && x != l1 - 1 && l2 != k4 && l2 != j2 - 1)
                                    {
                                        i3 = j4;
                                    }
                                }
                    ...
                    if (!flag3)
                    {
                        for (x = i4; x < l1; ++x)
                        {
                            double d13 = ((double)(x + chunkX * 16) + 0.5D - p_151541_6_) / d6;

                            for (z = k4; z < j2; ++z)
                            {
                                double d14 = ((double)(z + chunkZ * 16) + 0.5D - p_151541_10_) / d6;
                                int index = (x * 16 + z) * 256 + i2;
                                boolean foundTop = false;

                                if (d13 * d13 + d14 * d14 < 1.0D)
                                {
                                    for (int y = i2 - 1; y >= j4; --y)
                                    {
                                        double d12 = ((double)y + 0.5D - p_151541_8_) / d7;

                                        if (d12 > -1.7D && d13 * d13 + d12 * d12 + d14 * d14 < 2.5D)
                                        {
                                            Block block1 = data[index];

                                            if (isTopBlock(data, index, x, y, z, chunkX, chunkZ))
                                                foundTop = true;
                                            
                                            digBlock(data, index, x, y, z, chunkX, chunkZ, foundTop);
                                        }
    ...
    @Override
    protected void func_151538_a(World world, int chunkX, int chunkZ, int x, int z, Block[] data) {
        ...
        for (int j1 = 0; j1 < i1; ++j1)
        {
            ...
            {
                this.func_151542_a(this.rand.nextLong(), x, z, data, d0, d1, d2);
            ...
            for (int l1 = 0; l1 < k1; ++l1)
            {
                ...
                this.func_151541_a(this.rand.nextLong(), x, z, data, d0, d1, d2, f2, f, f1, 0, 0, 1.0D);
            }
        }
    }
    ...
    private boolean isTopBlock(Block[] data, int index, int x, int y, int z, int chunkX, int chunkZ)
    {
        ...
        return (isExceptionBiome(biome) ? data[index] == ModBlocks.grass : data[index] == biome.topBlock);
    }
    ...
    {
        BiomeGenBase biome = worldObj.getBiomeGenForCoords(x + chunkX * 16, z + chunkZ * 16);
        Block top    = (isExceptionBiome(biome) ? ModBlocks.grass : biome.topBlock);
        Block filler = (isExceptionBiome(biome) ? ModBlocks.dirt  : biome.fillerBlock);
        Block block  = data[index];

        if (block == ModBlocks.multiBlock1 || block == filler || block == top)
        {
            if (y < 10)
            {
