...
public class WorldGenCordaites extends WorldGeneratorCarboniferous
{
                    ...
                    for (int j1 = -4; j1 < 5; j1++)
                    {
                        for (int k1 = 0; k1 < 1; k1++)
                        {
                            for (int i2 = 0; i2 < 28 + random.nextInt(6); i2++)
                            {
                                ...
                                {
                                   this.setBlockandMetadataIfChunkExists(world, i + k1, j + i2, k + k2, ModBlocks.logs_1.blockID, 2);
                                }
                            }
                        }

                        if (this.getBlockIfChunkExists(world, i + l, (j + i1) - 1, k + j1) == ModBlocks.logs_1.blockID && isAirBlockIfChunkExists(world, i + l, j + i1, k + j1))
                        {
                            for (int l1 = -2; l1 < 3; l1++)
                            {
                                for (int j2 = -2; j2 < 3; j2++)
                                {
                                    for (int l2 = -2; l2 < 3; l2++)
                                    {
                                        ...
                                        {
                                           this.setBlockandMetadataIfChunkExists(world, i + l + l1, j + i1 + j2, k + j1 + l2, ModBlocks.leaves_1.blockID, 2);
                                        }
                                    }
                                }
                            }
    ...
    private boolean generateBranches(World world, Random random, int i, int j, int k, int l, int i1,
            ...
            int i3)
    {
        if (isAirBlockIfChunkExists(world, i, j, k) && this.getBlockIfChunkExists(world, i - l, j - 1, k - i2) == ModBlocks.logs_1.blockID && world.getBlockMetadata(i - l, j - 1, k - i2) == 2 && random.nextInt(10) == 0)
        {
           this.setBlockandMetadataIfChunkExists(world, i, j, k, ModBlocks.logs_1.blockID, 2);
           this.setBlockandMetadataIfChunkExists(world, i + l, j + 1, k + i2, ModBlocks.logs_1.blockID, 2);
           this.setBlockandMetadataIfChunkExists(world, i + i1, j + 2, k + j2, ModBlocks.logs_1.blockID, 2);
            this.setBlockandMetadataIfChunkExists(world, i + j1, j + 3, k + k2, ModBlocks.logs_1.blockID, 2);
           this.setBlockandMetadataIfChunkExists(world, i + k1, j + 4, k + l2, ModBlocks.logs_1.blockID, 2);
           this.setBlockandMetadataIfChunkExists(world, i + l1, j + 5, k + i3, ModBlocks.logs_1.blockID, 2);
            for (int j3 = -2; j3 < 3; j3++)
            {
                for (int k3 = -2; k3 < 3; k3++)
                {
                    for (int l3 = -2; l3 < 3; l3++)
                    {
                        ...
                        {
                           this.setBlockandMetadataIfChunkExists(world, j4 + j3, k4 + k3, l4 + l3, ModBlocks.leaves_1.blockID, 2);
                        }
                    }
                }
            }
