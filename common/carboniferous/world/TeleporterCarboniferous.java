...
public class TeleporterCarboniferous extends Teleporter {
	...
	public boolean placeInExistingPortal(Entity entity, double x, double y, double z, float par8) {
        ...
        for (int j1 = posX - var1; j1 <= posX + var1; j1++) {
            ...
            for (int j2 = posZ - var1; j2 <= posZ + var1; j2++) {
                ...
                for (int k2 = 127; k2 >= 0; k2--) {
                    if (worldObj.getBlockId(j1, k2, j2) != ModBlocks.portal.blockID) {
                        continue;
                    }
                    for (; worldObj.getBlockId(j1, k2 - 1, j2) == ModBlocks.portal.blockID; k2--) { }
        ...
        if (d >= 0.0D) {
            ...
            if (worldObj.getBlockId(k1 - 1, l1, i2) == ModBlocks.portal.blockID) {
            	newEntityPosX -= 0.5D;
            }
            if (worldObj.getBlockId(k1 + 1, l1, i2) == ModBlocks.portal.blockID) {
            	newEntityPosX += 0.5D;
            }
            if (worldObj.getBlockId(k1, l1, i2 - 1) == ModBlocks.portal.blockID) {
            	newEntityPosZ -= 0.5D;
            }
            if (worldObj.getBlockId(k1, l1, i2 + 1) == ModBlocks.portal.blockID) {
            	newEntityPosZ += 0.5D;
	                        ...
	                        for (int k7 = 0; k7 < 3; k7++)
	                        {
	                            for (int j8 = 0; j8 < 4; j8++)
	                            {
	                                for (int i9 = -1; i9 < 4; i9++)
	                                {
	                                    ...
	                                    if (i9 < 0 && !worldObj.getBlockMaterial(k9, i10, k10).isSolid() || i9 >= 0 && !worldObj.isAirBlock(k9, i10, k10))
	                                    {
	                                        break label0;
	                                    }
	                                }
	                        ...
	                        for (int k5 = l1; k5 < l1 + 2; k5++)
	                        {
	                            ...
	                            for (int l7 = 0; l7 < 4; l7++)
	                            {
	                                for (int k8 = -1; k8 < 4; k8++)
	                                {
	                                    ...
	                                    int l9 = l4 + k8;
	                                    int j10 = k3 + (l7 - 1) * i7;
	                                    if (k8 < 0 && !worldObj.getBlockMaterial(j9, l9, j10).isSolid() || k8 >= 0 && !worldObj.isAirBlock(j9, l9, j10))
	                                    {
	                                        break label1;
	                                    }
	                                }
	        ...
	        for (int i5 = 0; i5 < 4; i5++)
	        {
	            ...
	            for (int l5 = 0; l5 < 4; l5++)
	            {
	                for (int k6 = -1; k6 < 4; k6++)
	                {
	                    ...
	                    int i8 = i3 + k6;
	                    int l8 = l3 + (l5 - 1) * j4;
	                    worldObj.notifyBlocksOfNeighborChange(j7, i8, l8, worldObj.getBlockId(j7, i8, l8));
	                }
	            }
	        }
	...
	public static void formPortal(World world, int x, int y, int z, boolean build) {
		...
		world.setBlock(x, y - 1, z, ModBlocks.timeBox.blockID, 0, 3);
		if(build) {
			world.setBlock(x, y - 1, z, ModBlocks.timeBox.blockID, 1, 3);
			for(int var1 = 0; var1 <= 1; ++var1) {
				world.setBlock(x, y + var1, z, ModBlocks.portal.blockID);
				for(int var2 = -1; var2 <= 1; ++var2) {
					for(int var3 = -1; var3 <= 1; ++var3) {
						boolean flag = var2 != 0 && var3 != 0;
						if(flag) {
							world.setBlock(x + var2, y + var1, z + var3, Block.stoneBrick.blockID);
						}
						
						if(var1 == 1) {
							world.setBlock(x + var2, y + 2, z + var3, Block.stoneSingleSlab.blockID, 5, 3);
						}
					}
				}
			}
			
			world.setBlock(x, y + 2, z, Block.stoneSingleSlab.blockID, 13, 3);
			
			world.setBlock(x + 2, y, z + 1, Block.stairsStoneBrick.blockID, 1, 3);
			world.setBlock(x + 2, y, z - 1, Block.stairsStoneBrick.blockID, 1, 3);
			world.setBlock(x + 2, y, z - 2, Block.stairsStoneBrick.blockID, 1, 3);
			world.setBlock(x + 2, y, z + 2, Block.stairsStoneBrick.blockID, 1, 3);
			
			world.setBlock(x - 2, y, z + 1, Block.stairsStoneBrick.blockID, 0, 3);
			world.setBlock(x - 2, y, z - 1, Block.stairsStoneBrick.blockID, 0, 3);
			world.setBlock(x - 2, y, z - 2, Block.stairsStoneBrick.blockID, 0, 3);
			world.setBlock(x - 2, y, z + 2, Block.stairsStoneBrick.blockID, 0, 3);
	
			world.setBlock(x + 1, y, z - 2, Block.stairsStoneBrick.blockID, 2, 3);
			world.setBlock(x - 1, y, z - 2, Block.stairsStoneBrick.blockID, 2, 3);
			
			world.setBlock(x + 1, y, z + 2, Block.stairsStoneBrick.blockID, 3, 3);
			world.setBlock(x - 1, y, z + 2, Block.stairsStoneBrick.blockID, 3, 3);
		}
	}
}
