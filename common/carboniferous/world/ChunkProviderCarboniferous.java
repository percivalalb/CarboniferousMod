...
import carboniferous.world.structure.MapGenSpiderLair;
...
public class ChunkProviderCarboniferous implements IChunkProvider
{
    private Random rand;
    ...
    //Custom Variables
    public static byte seaHeight = 63;
    ...
    public void generateTerrain(int par1, int par2, byte[] blocks, byte[] metadata) {
        byte b0 = 4;
        byte b1 = 16;
        byte b2 = seaHeight;
        int k = b0 + 1;
                    ...
                    for (int l1 = 0; l1 < 8; ++l1)
                    {
                        ...
                        for (int i2 = 0; i2 < 4; ++i2)
                        {
                            ...
                            for (int k2 = 0; k2 < 4; ++k2)
                            {
                                if ((d16 += d15) > 0.0D)
                                {
                                    blocks[j2 += short1] = (byte)ModBlocks.multiBlock1.blockID;
                                }
                                else if (k1 * 8 + l1 < b2)
                                {
                                    blocks[j2 += short1] = (byte)Block.waterStill.blockID;
                                }
                                else
                                {
                                    blocks[j2 += short1] = 0;
    ...
    public void replaceBlocksForBiome(int par1, int par2, byte[] blocks, byte[] metadata, BiomeGenBase[] biomeArray) {
        byte b0 = seaHeight;
        double d0 = 0.03125D;
        ...
        for (int k = 0; k < 16; ++k)
        {
            for (int l = 0; l < 16; ++l)
            {
                ...
                int j1 = -1;
                byte b1 = biomegenbase.topBlock; //Top Block (ID)
                byte b2 = biomegenbase.fillerBlock; //Filler Block (ID)
                byte b4 = 0; //Top Block (Metadata)
                byte b5 = 0; //Filler Block (Metadata)
                
                if(biomegenbase instanceof BiomeGenBaseCarboniferous) {
                	b4 = ((BiomeGenBaseCarboniferous)biomegenbase).topBlockMeta;
                	b5 = ((BiomeGenBaseCarboniferous)biomegenbase).fillerBlockMeta;
                }
                ...
                for (int k1 = 127; k1 >= 0; --k1) {
                    ...
                    else
                    {
                        ...
                        else if (b3 == (byte)ModBlocks.multiBlock1.blockID) {
                            if (j1 == -1) {
                                if (i1 <= 0) {
                                    b1 = 0;
                                    b2 = (byte)ModBlocks.multiBlock1.blockID;
                                }
                                ...
                                if (j1 == 0 && b2 == (byte)Block.sand.blockID) {
    ...
    public Chunk provideChunk(int par1, int par2) {
        ...
        this.generateTerrain(par1, par2, blocks, metadata);
        this.biomesForGeneration = this.worldObj.getWorldChunkManager().loadBlockGeneratorData(this.biomesForGeneration, par1 * 16, par2 * 16, 16, 16);
        this.replaceBlocksForBiome(par1, par2, blocks, metadata, this.biomesForGeneration);
        this.caveGenerator.generate(this, this.worldObj, par1, par2, blocks);
        
        if (this.mapFeaturesEnabled) {
            this.scatteredFeatureGenerator.generate(this, this.worldObj, par1, par2, blocks);
        }
