...
import carboniferous.ModEntitys;
...
import carboniferous.tileentity.TileEntityCarboniferous;
...
public class ClientProxy extends CommonProxy {
	...
	public void onModLoad() {
		MinecraftForgeClient.registerItemRenderer(ModBlocks.grinder.blockID, new ItemGrinderRenderer());
		MinecraftForgeClient.registerItemRenderer(ModBlocks.compressor.blockID, new ItemCompressorRenderer());
	}
    ...
    public void handleTileEntityPacket(int x, int y, int z, ForgeDirection direction, String owner, String customName, String state) {
        ...
        if (tileEntity != null) {
            if (tileEntity instanceof TileEntityCarboniferous) {
                ((TileEntityCarboniferous)tileEntity).setDirection(direction);
                ((TileEntityCarboniferous)tileEntity).setState(state);
                ((TileEntityCarboniferous)tileEntity).setOwner(owner);
                ((TileEntityCarboniferous)tileEntity).setInvName(customName);
            }
        }
    }
	...
	public void handleWallShellPacket(int x, int y, int z, int id, int metadata) {
	    ...
	    if (tileEntity != null) {
	       if (tileEntity instanceof TileEntityWallShell) {
	    	   ((TileEntityWallShell)tileEntity).setShellIdAndMeta(id, metadata);
	       }
	    }
	}
