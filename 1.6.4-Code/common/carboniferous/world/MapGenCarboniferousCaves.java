...
public class MapGenCarboniferousCaves extends MapGenBase
{
    ...
    private boolean isTopBlock(byte[] data, int index, int x, int y, int z, int chunkX, int chunkZ) {
        ...
        return (isExceptionBiome(biome) ? data[index] == ModBlocks.grass.blockID : data[index] == biome.topBlock);
    }
    ...
    protected void digBlock(byte[] data, int index, int x, int y, int z, int chunkX, int chunkZ, boolean foundTop) {
        BiomeGenBase biome = worldObj.getBiomeGenForCoords(x + chunkX * 16, z + chunkZ * 16);
        int top    = (isExceptionBiome(biome) ? ModBlocks.grass.blockID : biome.topBlock);
        int filler = (isExceptionBiome(biome) ? ModBlocks.dirt.blockID  : biome.fillerBlock);
        int block  = data[index];

        if (block == (byte)ModBlocks.multiBlock1.blockID || block == filler || block == top) {
            if (y < 10) {
